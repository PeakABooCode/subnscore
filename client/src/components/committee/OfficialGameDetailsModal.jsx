        ...
        <div className="p-6 overflow-y-auto custom-scrollbar space-y-8">
          ...
          <div className="grid grid-cols-3 items-center gap-4 bg-slate-50 p-6 rounded-2xl border border-slate-100">
            ...
            <div className="flex flex-col items-center">
              <span className="text-[8px] font-black bg-slate-900 text-white px-3 py-1 rounded-full uppercase mb-2">
                FINAL RESULT
              </span>
            ...
            <div className="text-center">
              <p className="text-[10px] font-black text-red-500 uppercase tracking-widest mb-1 truncate">
                {game.team_b_name}
              </p>
                  ...
                  <div key={log.id || idx} className="flex items-center justify-between p-4 hover:bg-slate-50 transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <span className={`text-[10px] font-black uppercase ${log.team_side === 'A' ? 'text-blue-600' : 'text-red-600'}`}>
                          {log.team_side === 'A' ? game.team_a_name : game.team_b_name}
                        </span>
                        <span className="text-sm font-bold text-slate-800">
                          {log.player_id ? `#${log.jersey} ${log.player_name}` : 'TEAM ACTION'}
                        </span>
                      </div>
                    </div>
                    ...
                    <div>
                      <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${
                        ...
                        {log.action_type === 'SCORE' ? `+${log.amount} POINTS` :
                         log.action_type === 'FOUL' ? 'FOUL' :
